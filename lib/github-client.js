...
GithubClient.prototype = {
    ...
    return this.get(
      ...
      .catch(function (error) {
        if (error.statusCode === 404) {
          throw new Error("Can't find post file: " + path);
        } else {
          throw error;
        }
      })
  ...
  listPullRequests: function (user, repo, base, state) {
    invariant(state === 'open' || state === 'closed' || state === 'all', "State must be open, closed or all");
    return this.get(
      "/repos/{user}/{repo}/pulls?base={base}&state={state}",
      {user: user, repo: repo, base: base, state: state})
    .then(function (response) {
      if (response.statusCode !== 200) {
          throw new Error("Got unexpected status code when listing pull requests: " + response.statusCode);
      }
      return response.body;
    });
  },
